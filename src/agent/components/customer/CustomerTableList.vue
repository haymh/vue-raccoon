<template>
  <div>
    ...
    <v-card>
      ...
      <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search" v-model="selected" selected-key="id" select-all class="elevation-1">
    ...
    <v-dialog lazy v-model="showCreateContact" persistent fullscreen transition="dialog-bottom-transition" :overlay=false>
      <CreateContact :showExitButton="true" :exitAction="closeCreateContact" :toEdit="toEdit"></CreateContact>
    </v-dialog>
  </div>
